//INTEGRACION MEMORIA RETENTIVA
#include <EEPROM.h> // Almacenar datos en la memoria permanente del Arduino
int Addr = 0;  //Necesaria para indicar la posición en memoria
...
uint32_t T_S1_UP = 10; // [ms] Tiempo para detección de pieza
uint32_t T_S1_DOWN = 10; // [ms] Tiempo entre piezas
 // INCLUDE
// #include <SPI.h>
// #include <Ethernet.h>
...
// INTEGRACION MEMORIA RETENTIVA
    //Obtener datos almacenado en memoria estática
      Addr = 0;
      EEPROM.get( Addr, contador_S1_ISR );  //Obtener total de piezas producidas.
      delay(10);
...
//-----------------------------------
  ...
  if (digitalRead(Sensor1_pin)==HIGH && S1_HIGH==false){
    millis_prev=millis();
    while(digitalRead(Sensor1_pin)==HIGH){
  ...
  if (S1_paso2){
    if (S1_LOW){
      ...
      S1_paso2=false;
      S1_paso3=true;
      // Serial.println("paso 2 ejecutado");
    }
  }
  if (S1_paso3){
    if (S1_LOW){
      T22_S1=millis();
      if (T22_S1-T21_S1>T_S1_UP){
        estado_S1=1;
        S1_paso3=false;
        S1_paso0=true;
...
//------------------------------------
//IMPLEMENTACIÓN RESET CONTADOR
 if (contador_S1_ISR>32000){
  contador_S1_ISR=0;
 }
 if (contador_S2_ISR>32000){
  contador_S2_ISR=0;
 }
 //----------------------------------
//  mqttLoop();
//Transmisión de mensajes para sensor 1  
  if (estado_S1==0 && (millis()-T_S1_anterior>T_S1_inactividad)){
    ...
    Serial.println(msg_aux);
    //-------------------------------------
    //IMPLEMENTACIÓN MEMORIA RETENTIVA
    Addr = 0;
    EEPROM.put( Addr, contador_S1_ISR );  //Grabamos el valor
    //-------------------------------------
    estado_S1=0;
    contador_S1++;
    Serial.println("??KEEPALIVE");
